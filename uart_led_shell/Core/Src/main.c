...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  HAL_UART_Transmit(&huart2, (uint8_t *)"UART CMD READY\r\n", 16, HAL_MAX_DELAY);
  HAL_UART_Transmit(&huart2, (uint8_t *)"Commands: ON / OFF / TOG\r\n", 27, HAL_MAX_DELAY);
  ...
  while (1)
  {
      ...
      if (rx_char == '\r' || rx_char == '\n')
      {
          ...
          else if (strcmp((char *)rx_buf, "TOG") == 0)
          {
          ...
          else if (strcmp((char *)rx_buf, "HELP") == 0)
          {
              ...
              HAL_UART_Transmit(&huart2, (uint8_t *)"ON     - turn LED on\r\n", 22, HAL_MAX_DELAY);
              HAL_UART_Transmit(&huart2, (uint8_t *)"OFF    - turn LED off\r\n", 23, HAL_MAX_DELAY);
              HAL_UART_Transmit(&huart2, (uint8_t *)"TOG    - toggle LED\r\n", 22, HAL_MAX_DELAY);
              HAL_UART_Transmit(&huart2, (uint8_t *)"STATUS - show LED state\r\n", 25, HAL_MAX_DELAY);
          }
          else if (strcmp((char *)rx_buf, "STATUS") == 0)
          {
              if (led_state)
              {
                  HAL_UART_Transmit(&huart2, (uint8_t *)"LED STATE: ON\r\n", 15, HAL_MAX_DELAY);
              }
              else
              {
                  HAL_UART_Transmit(&huart2, (uint8_t *)"LED STATE: OFF\r\n", 16, HAL_MAX_DELAY);
              }
          }
          else if (idx > 0)
          {
